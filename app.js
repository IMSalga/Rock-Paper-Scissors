// Detection of the result by using 2 params! Your choice and PC choice[Rock, papaer and scissors]
class Result {
    static whoWin(yourChoice, PCchoice) {
      if (yourChoice === "rock" && PCchoice === "scissors" 
      || yourChoice === "scissors" && PCchoice === "paper" 
      || yourChoice === "paper" && PCchoice === "rock") return "win";
      else if (yourChoice === "scissors" && PCchoice === "rock" 
      || yourChoice === "paper" && PCchoice === "scissors" 
      || yourChoice === "rock" && PCchoice === "paper") return "lose";
      else return "draw";
    }
  }
  
  // generates the choice randomly
  class Choice {
  ...
  // keeping the records of the scores/stats
  class Stats {
  ...
  // Global app Game where the game takes place
  class Game {
    constructor() {
    ...
    startGame(e) {
      ...
      if (yourChoice === PcChoice) {
        [...this.optionsImg].find(choice => choice.dataset.option === PcChoice 
          && choice.dataset.option === PcChoice).classList.add('draw-color');
        this.draw.classList.toggle('draw-animation');
      } else {
      [...this.optionsImg].find(choice => choice.dataset.option 
        === yourChoice).classList.add('player-choice');
        [...this.optionsImg].find(choice => choice.dataset.option 
          === PcChoice).classList.add('pc-choice');
      }
